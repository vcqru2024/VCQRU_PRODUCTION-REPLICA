...
public partial class FrmDiscountMaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["User_Type"] == null)
            Response.Redirect("Login.aspx?Page=FrmDiscountMaster.aspx");
        else
        {
    ...
    {                
        ...
        FillPlan(Reg);
        newMsg.Visible = false;
        lblheading.Text = "Create New Plan Discount";
    ...
    protected void GrdLabel_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (Arr.Length > 1)
        {
            ...
            if (e.CommandName == "EditLabel")
            {
                ...
                btnSave.Text = "Update";
                lblheading.Text = "Update Plan Discount";
                ModalPopupCreateLabel.Show();
            }
            else if (e.CommandName == "ShowHideLabel")
            {
                ...
                lblPopMsgText.Text = "Are you sure to " + FindStatus(Convert.ToInt32(ds.Tables[0].Rows[0]["Flag"].ToString())) + " <span style='color:blue;' >" + ds.Tables[0].Rows[0]["Plan_Name"].ToString() + " </span>  Plan Discount ?";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (btnSave.Text == "Save")
        {
            ...
            if (function9420.CheckDateFromDiscount(Reg))
            {
                ...
                ProductsLabelPrices.Text = "Please select valid AMC Plan date from";
                ModalPopupCreateLabel.Show();
            ...
            if (function9420.CheckDateToDiscount(Reg))
            {
                Div1.Visible = true;
                Div1.Attributes.Add("class", "alert_boxes_pink");
                ProductsLabelPrices.Text = "Please select valid AMC Plan date to";
                ModalPopupCreateLabel.Show();
                script = @"setTimeout(function(){document.getElementById('" + Div1.ClientID + "').style.display='none';},5000);";
                Page.ClientScript.RegisterStartupScript(this.GetType(), "somekey", script, true);
                return;
            ...
            if (Reg.DateFrom >= Reg.DateTo)
            {
                Div1.Visible = true;
                Div1.Attributes.Add("class", "alert_boxes_pink");
                ProductsLabelPrices.Text = "Please select valid AMC Plan date from your old Amc End Date : - " + Convert.ToDateTime(Reg.DateTo).ToString("dd/MM/yyyy");
                ModalPopupCreateLabel.Show();
                script = @"setTimeout(function(){document.getElementById('" + Div1.ClientID + "').style.display='none';},5000);";
                Page.ClientScript.RegisterStartupScript(this.GetType(), "somekey", script, true);
                return;
