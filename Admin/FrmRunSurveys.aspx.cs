...
public partial class FrmRunSurveys : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["User_Type"] == null)
            Response.Redirect("Login.aspx?Page=FrmRunSurveys.aspx");
        else
        {
            if (Session["User_Type"].ToString() == "Company")
                Response.Redirect("../Info/Login.aspx?Page=BigDataAnalysis.aspx");
        }
        if (!Page.IsPostBack)
        {
            FillCompany();
    ...
    public DataTable FillDataaTable()
    {
        ...
        DataSet ds = ExecuteNonQueryAndDatatable.GetConsumerList("", ddlproname.SelectedValue, ddlcompname.SelectedValue, dtDuedate, dtDueTo, "", "runsurvey");
    ...
    private void FillCompany()
    {
        ...
        DataProvider.MyUtilities.FillDDLInsertZeroIndexStringWithEmpty(ds, "Comp_ID", "Comp_Name", ddlcompname, "--Company--");
    ...
    //    downloadExcel(dt, "Run Surveys Details");
    //}
    //private void downloadExcel(DataTable dt, string FileName)
    //{
    //    Response.Clear();
    ...
    protected void ddlcompname_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlcompname.SelectedIndex > 0)
        {
            Reg.Comp_ID = ddlcompname.SelectedValue;
            DataSet ds = SQL_DB.ExecuteDataSet("SELECT p.Comp_ID, p.Pro_ID ,p.Pro_Name FROM Pro_Reg AS p WHERE (p.Doc_Flag = 1) AND (p.Sound_Flag = 1) AND (p.Pro_ID IN (SELECT s.Pro_ID FROM M_ServiceSubscription as s " +
            " WHERE (''='" + ddlcompname.SelectedValue.Trim().ToString() + "' OR s.comp_ID='" + ddlcompname.SelectedValue.Trim().ToString() + "') AND  (s.Service_ID IN (SELECT m.Service_ID FROM M_ServiceFeature as m WHERE (m.IsNotify = 0)))))");
    ...
    //        }
    //    }
    //    string Qry = "SELECT tg.RowId, tg.Pro_ID,(SELECT pr.Pro_Name FROM Pro_Reg AS pr WHERE pr.Pro_ID = tg.Pro_ID) as Pro_Name, tg.Rating,(SELECT mt.SurveystypeText FROM M_RunSurveys AS mt WHERE mt.RowId = tg.Rating) as SurveystypeText, tg.MobileNo, tg.EntryDate FROM T_RunSurveys as tg " + NewQry + " order by tg.EntryDate desc";
    //    #endregion
    //    DataSet ds = SQL_DB.ExecuteDataSet(Qry);
    //    #endregion
    //    if (Convert.ToInt32(ddlRows.SelectedValue) > 1000)
