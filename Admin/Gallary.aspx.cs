...
public partial class Admin_ProductMaster : System.Web.UI.Page
{
    ...
    protected void imgBtnSubmit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        //{
        //    string Pro_Id = SQL_DB.ExecuteScalar("SELECT [PrPrefix] +convert(nvarchar,[PrStart]) as id FROM [Code_Gen] where [Prfor] = 'Album'").ToString(); // DataProvider.Uttils.GetNewCode("Album");           
        //    SQL_DB.ExecuteNonQuery("INSERT INTO [M_Album] ([Album_Id],[Album_Name]) VALUES('" + Pro_Id + "' ,'" +Album_Name + "')");
        //    if (Request.Files.Count > 0)
        //        SaveFile(Pro_Id);
        //    SQL_DB.ExecuteNonQuery("update [Code_Gen] set [PrStart] = [PrStart] + 1 where [Prfor] = 'Album'");  
        //    //DataProvider.Uttils.SetNewCode("Album");
    ...
    {
        //DataSet ds = new DataSet();
        //SQL_DB.GetDA("SELECT [Album_Id],[Album_Name],'Gallary/'+(SELECT top(1) [Image_Name] FROM [T_Album_Image] where [Album_Id]=[M_Album].Album_Id) as image_name FROM [M_Album] where Album_Id in (SELECT Album_Id FROM [T_Album_Image]) and Album_Name like '%"+txtSearchProName.Text.Trim()+"%'").Fill(ds, "1");
        //GridView1.PageSize = Convert.ToInt32(ddlpagesize.SelectedValue);
        //lblcnt.Text = ds.Tables["1"].Rows.Count.ToString();
    ...
    //    {
    //        DataSet ds = new DataSet();           
    //        SQL_DB.GetDA("SELECT [Album_Id],[Album_Name] FROM [M_Album] where Album_Id='" + lblprdid.Text + "'").Fill(ds, "1");
    //        if (ds.Tables["1"].Rows.Count > 0)
    //        {
    ...
    //            RequiredFieldValidator3.ValidationGroup = "xyz";
    //            ds.Clear();
    //            SQL_DB.GetDA("SELECT [Album_Id],[Image_Name] FROM [T_Album_Image] where Album_Id='" + e.CommandArgument.ToString() + "'").Fill(ds, "1");
    //            List<UploadeFile> FDetail = new List<UploadeFile>();
    //            Session["UploadeFileList"] = FDetail;
    //            UploadeFile f;
    //            StringBuilder str = new StringBuilder();
