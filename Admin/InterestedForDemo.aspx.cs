...
public partial class InterestedForDemo : System.Web.UI.Page
{
    ...
    private void FillData()
    {
        ...
        if ((txtDateFrom.Text != "") && (txtDateFrom.Text != ""))
            Reg.statusstr = " AND CONVERT(VARCHAR,Reg_Date,111) >= '" + Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy/MM/dd") + "' AND CONVERT(VARCHAR,Reg_Date,111) <= '" + Convert.ToDateTime(txtDateTo.Text).ToString("yyyy/MM/dd") + "'";
        else if ((txtDateFrom.Text != "") && (txtDateFrom.Text == ""))
            Reg.statusstr = " AND CONVERT(VARCHAR,Reg_Date,111) >= '" + Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy/MM/dd") + "' ";
        else if ((txtDateFrom.Text == "") && (txtDateFrom.Text != ""))
            Reg.statusstr = " AND CONVERT(VARCHAR,Reg_Date,111) <= '" + Convert.ToDateTime(txtDateTo.Text).ToString("yyyy/MM/dd") + "' ";
        //if (Session["User_Type"] != null)
        //{
        ...
        //        Reg.Comp_ID = "";
        //}
        Reg.Comp_Name = txtcompname.Text.Trim().Replace("'", "''");
        Reg.chkstr = txtClientname.Text.Trim().Replace("'", "''");
        ds = function9420.FillInterestedForDemo(Reg);
    ...
    {
        for (int i = 0; i < GrdVwTestimonial.Rows.Count; i++)
        {
            if (GrdVwTestimonial.DataKeys[i]["Status1"].ToString() == "-1")
            {
                System.Drawing.Color col = System.Drawing.Color.FromName("#FCEEEE");
                GrdVwTestimonial.Rows[i].BackColor = col;
            }
            else if (GrdVwTestimonial.DataKeys[i]["Status1"].ToString() == "1")
            {
