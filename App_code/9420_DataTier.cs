...
namespace Data9420
{
    public class Data_9420
    {
        ...
        public static DataSet FetchSearchData(Object9420 obj)
        {
            ...
            #region Changed as patanjali required
           ...
           " ,'../Data/Sound/' + SUBSTRING(dbo.Pro_Reg.Comp_ID, 6, 4) + '/' + dbo.Pro_Reg.Pro_ID + '/' + Pro_Reg.Pro_Doc AS DocPath " +
           " ,(SELECT m.Plan_Name FROM  M_Amc_Offer AS t INNER JOIN  M_Plan AS m ON t.Plan_ID = m.Plan_ID WHERE (t.Pro_ID = dbo.Pro_Reg.Pro_ID) AND (t.Amc_Offer_ID = (SELECT max(mt.Amc_Offer_ID) FROM M_Amc_Offer as mt WHERE (mt.Status = 1) AND (mt.Pro_ID = dbo.Pro_Reg.Pro_ID)))) as Plan_Name " +
           " ,ISNULL((SELECT DATEDIFF(DAY,GETDATE(),Date_To) FROM M_Amc_Offer WHERE Amc_Offer_ID = (SELECT MAX(Amc_Offer_ID) FROM M_Amc_Offer WHERE (Status = 1) AND (Pro_ID = dbo.Pro_Reg.Pro_ID))),0) AS  CntDays, case when (pro_reg.Doc_Flag=1 and Sound_Flag=1) then 'Verified' else 'Pending' end as Status " +
           " FROM dbo.Pro_Reg LEFT OUTER JOIN" +
           " dbo.M_Label ON dbo.Pro_Reg.Label_Code = dbo.M_Label.Label_Code" +
           " WHERE ('' = '" + obj.Comp_ID + "' or dbo.Pro_Reg.Comp_ID = '" + obj.Comp_ID + "') AND (dbo.Pro_Reg.Pro_Name LIKE '%" + obj.Pro_Name + "%') AND ('' = '" + obj.Pro_ID + "' or dbo.Pro_Reg.Pro_ID = '" + obj.Pro_ID + "') ORDER BY  dbo.Pro_Reg.Pro_Entry_Date DESC";
        ...
        public static void FillUpDateProfile(Object9420 Reg)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    Reg.PAN_TAN = ds.Tables[0].Rows[0]["PAN_TAN"].ToString();
                    Reg.VAT = ds.Tables[0].Rows[0]["VAT"].ToString();
                }
        ...
        {
            if (obj.Comp_ID == "Comp-1693")
                dbCommand = database.GetSqlStringCommand("SELECT COUNT(Print_Status) FROM [M_Code_PFL] WHERE [Pro_ID] ='" + obj.Pro_ID + "' AND [Print_Status] = 1");
            else
        ...
        public static string FilCodelInfo(string Compid)
        {
            ...
            database.AddInParameter(dbCommand, "@companyid", DbType.String, Compid);
        ...
        {
            if (Reg.Comp_ID == "Comp-1693")
                dbCommand = database.GetSqlStringCommand("UPDATE [M_Code_PFL] SET [Print_Date] = '" + Convert.ToDateTime(DataProvider.LocalDateTime.Now).ToString("MM/dd/yyyy") + "',[Allot_Date] = '" + Convert.ToDateTime(DataProvider.LocalDateTime.Now).ToString("MM/dd/yyyy") + "'" +
        ...
        public static string MyRegistrationExccel_ImageFile(Object9420 RegExccel)
        {
            string Qry = "";
            if (RegExccel.Comp_ID == "Comp-1693")
            {
                Qry = "SELECT row_number() OVER (ORDER BY Excel.Series_Order,Excel.Searies_Name) AS SNO,* from (SELECT Pro_ID +'-'+ (case when len(convert(nvarchar,Series_Order)) = 1 then '000'+convert(nvarchar,Series_Order) when len(convert(nvarchar,Series_Order)) = 2 then '00'+convert(nvarchar,Series_Order) when len(convert(nvarchar,Series_Order)) = 3 then '0'+convert(nvarchar,Series_Order) else convert(nvarchar,Series_Order) end)+'-'+ " +
          " (case when len(convert(nvarchar,Series_Serial)) = 1 then '000'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 2 then '00'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 3 then '0'+convert(nvarchar,Series_Serial) else convert(nvarchar,Series_Serial) end) AS Searies_Name ,Code1, Code2 ,Series_Order, CONCAT(Code1,'-', Code2) AS ImageFile FROM  M_Code_PFL " +
          " WHERE CONVERT(VARCHAR,Print_Date,111)='" + DataProvider.LocalDateTime.Now.ToString("yyyy/MM/dd") + "' AND Pro_ID='" + RegExccel.Pro_ID + "' and print_status = 1 AND (LabelRequestId = '" + RegExccel.LabelRequestID + "' ) " +
          " ) as Excel";
            }
            else
            {
                Qry = "SELECT row_number() OVER (ORDER BY Excel.Series_Order,Excel.Searies_Name) AS SNO,* from (SELECT Pro_ID +'-'+ (case when len(convert(nvarchar,Series_Order)) = 1 then '000'+convert(nvarchar,Series_Order) when len(convert(nvarchar,Series_Order)) = 2 then '00'+convert(nvarchar,Series_Order) when len(convert(nvarchar,Series_Order)) = 3 then '0'+convert(nvarchar,Series_Order) else convert(nvarchar,Series_Order) end)+'-'+ " +
          " (case when len(convert(nvarchar,Series_Serial)) = 1 then '000'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 2 then '00'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 3 then '0'+convert(nvarchar,Series_Serial) else convert(nvarchar,Series_Serial) end) AS Searies_Name ,Code1, Code2 ,Series_Order, CONCAT(Code1,'-', Code2) AS ImageFile FROM  M_Code " +
          " WHERE CONVERT(VARCHAR,Print_Date,111)='" + DataProvider.LocalDateTime.Now.ToString("yyyy/MM/dd") + "' AND Pro_ID='" + RegExccel.Pro_ID + "' and print_status = 1 AND (LabelRequestId = '" + RegExccel.LabelRequestID + "' ) " +
          " ) as Excel";
            }
        ...
        public static int InsertCodeChecked(Object9420 Reg)
        {
            ...
            if (int.TryParse(Reg.Received_Code1, out n) && int.TryParse(Reg.Received_Code2, out n))
            {
                ...
                if (Reg.Comp_ID == "Comp-1693")
                    dbCommand = database.GetStoredProcCommand("PROC_InsertProductInquery_PFL");
                ...
                database.AddInParameter(dbCommand, "IsVerified", DbType.String, Reg.IsVerified);  //Added by Deep for Patanjali
                database.AddInParameter(dbCommand, "Latitude", DbType.String, Reg.Latitude);  //Added by Deep for Patanjali
                database.AddInParameter(dbCommand, "Longitude", DbType.String, Reg.Longitude);  //Added by Deep for Patanjali
                database.AddInParameter(dbCommand, "@PinCode", DbType.String, Reg.PinCode);//Added by Deep for Patanjali
                //  database.ExecuteNonQuery(dbCommand);
        ...
        {
            if (Reg.Comp_ID == "Comp-1693")
                dbCommand = database.GetStoredProcCommand("PROC_UpdateM_CodeUse_Count_PFL");
            else
        ...
        {
            dbCommand = database.GetSqlStringCommand("UPDATE [Pro_Reg] SET [Doc_Flag] = 1,[Sound_Flag]=1,[Doc_Remark]='Auto Approved',[Sound_Remark]='Auto Approved' WHERE [Pro_ID]='" + obj.Pro_ID + "'");
            database.ExecuteNonQuery(dbCommand);
        }
        ...
        public static DataSet FillProddlSearch(Object9420 Reg)
        {
            ...
            {
                if (Reg.Status == 1)
                    Qry = "  AND (M_Code_PFL.DispatchFlag = 1) ";
                dbCommand = database.GetSqlStringCommand("SELECT DISTINCT Pro_Reg.Pro_ID, Pro_Reg.Pro_Name, Pro_Reg.Comp_ID FROM M_Code_PFL FULL OUTER JOIN  Pro_Reg ON M_Code_PFL.Pro_ID = Pro_Reg.Pro_ID WHERE (M_Code_PFL.Print_Status = 1) " + Qry + "  AND (Pro_Reg.Comp_ID = '" + Reg.Comp_ID + "')");
            }
            else
            {
                if (Reg.Status == 1)
        ...
        public static DataSet FillFridForEnqDetails(Object9420 Reg)
        {
            ...
            //" AND cast([M_Code].Code2 as nvarchar) = [vw_Pro_Enq].[Received_Code2] left join [T_Pro] on [M_code].[Batch_No] = [T_Pro].[Row_ID] left join [m_consumer] mc on mc.mobileno = [vw_Pro_Enq].MobileNo " + Reg.Msg + " order by [vw_Pro_Enq].[Enq_Date] desc");
            #endregion
            if (Reg.Comp_ID == "Comp-1626")
            {
                dbCommand = database.GetSqlStringCommand("SELECT top 100 [M_Code].[Pro_ID],(SELECT [Pro_Name] FROM [Pro_Reg] Where [Pro_ID] = [M_Code].[Pro_ID]) as [Pro_Name], [vw_pro_enq_VendorReport_RamGopal].[Dial_Mode] AS ModeOfInquiry,CONVERT(varchar, [vw_pro_enq_VendorReport_RamGopal].[Enq_Date]) as EnquiryDate, [vw_pro_enq_VendorReport_RamGopal].[Mode_Detail] AS ContactDetails, [vw_pro_enq_VendorReport_RamGopal].[Received_Code1], " + //cast(convert(nvarchar,Pro_Enq.Enq_Date,109) as datetime) as EnquiryDate
           " [vw_pro_enq_VendorReport_RamGopal].[Received_Code2],ISNULL([vw_pro_enq_VendorReport_RamGopal].MobileNo,'-- --') as MobileNo,case when [M_Code].Pro_ID in ('AH11','AH12','AH13','AH14','AH15','AH79') and [vw_pro_enq_VendorReport_RamGopal].enq_date  between '2023-10-01 00:00:01.000' and '2023-12-01 23:59:59.000'" +
           " or([M_Code].Pro_ID in ('AC39', 'AC40', 'AC41', 'AC33', 'AC34', 'AC35', 'AC36', 'AC37') and[vw_pro_enq_VendorReport_RamGopal].enq_date >= '2021-04-01 00:00:00.000') " +
           "or([M_Code].Pro_ID in ('AG31', 'AG32', 'AG33', 'AG27', 'AG28', 'AG29', 'AG30') and[vw_pro_enq_VendorReport_RamGopal].enq_date >= '2022-04-01 00:00:00.000') then 'Failed' " +
           " when[vw_pro_enq_VendorReport_RamGopal].[Is_Success] = '1' then 'Success' when[vw_pro_enq_VendorReport_RamGopal].[Is_Success] = '0' then 'Invalid' else 'Unsuccess' end as Successstatus," +
           "isnull([vw_pro_enq_VendorReport_RamGopal].Network,'-- --') as Network,isnull([vw_pro_enq_VendorReport_RamGopal].Circle,'-- --') as Circle, isnull(mc.employeeid,'-- --') as employeeid , isnull(mc.distributorid,'-- --') as distributorid FROM (select Pro_ID,Code1,Code2 from M_Code (nolock) where Use_Count >0) [M_Code] INNER JOIN [vw_pro_enq_VendorReport_RamGopal] [vw_pro_enq_VendorReport_RamGopal] ON cast([M_Code].Code1 as nvarchar) = [vw_pro_enq_VendorReport_RamGopal].[Received_Code1] " +
           " AND cast([M_Code].Code2 as nvarchar) = [vw_pro_enq_VendorReport_RamGopal].[Received_Code2]  left join [m_consumer] mc on mc.mobileno = [vw_pro_enq_VendorReport_RamGopal].MobileNo " + Reg.Msg + " and coalesce(mc.isdelete,0)=0 order by [vw_pro_enq_VendorReport_RamGopal].[Enq_Date] desc");
            ...
            else
            {
               ...
               " AND cast([M_Code].Code2 as nvarchar) = [vw_Pro_Enq].[Received_Code2]  left join [m_consumer] mc on mc.mobileno = [vw_Pro_Enq].MobileNo " + Reg.Msg + " and coalesce(mc.isdelete,0)=0 and [vw_Pro_Enq].[IsDelete] = 0 order by [vw_Pro_Enq].[Enq_Date] desc");
            }
            dbCommand.CommandTimeout = 5000000;
            return database.ExecuteDataSet(dbCommand);
        }
        ...
        public static DataSet FillFridForEnqDetails_Warranty_download(Object9420 Reg)
        {
            ...
            string str = "";
            if (Reg.Comp_ID == "Comp-1629")
            {
                str = "select purchasedate as 'Purchase Date',C.Pro_Name +'('+ C.Pro_id + ')'  [Product Name (Product Id)], concat(B.code1,B.code2) as 'Complete Code',MOBILE 'Mobile No',case when datediff(month,Purchasedate,Expirationdate) >0 then 'Yes' else 'No' end as 'Is Warranty' ,ExpirationDate as 'Warranty Expiration Date',"
               + "datediff(month,Purchasedate,Expirationdate) 'Warranty Period (months)'" +
               " ,BillNo as [Vehicle Number] ,State as [Device], Comment as [User Comment],replace(ImagePath,'~/','https://www.vcqru.com/') as [Product Image], replace(ImagePathBill,'~/','https://www.vcqru.com/') as [Bill Invoice]  " +
               " from [dbo].[WarrentyDetails] A inner join m_code B on cast(B.Code1 as varchar(20)) +'-'+ cast(B.code2 as varchar(20)) = A.Code " +
               " inner join pro_reg C on C.Pro_id = B.Pro_id where  C. Comp_ID = '" + Reg.Comp_ID + "' order by Purchasedate desc";
            ...
            else
            {
                str = "select purchasedate as 'Purchase Date',C.Pro_Name +'('+ C.Pro_id + ')'  [Product Name (Product Id)], concat(B.code1,B.code2) as 'Complete Code',MOBILE 'Mobile No',case when datediff(month,Purchasedate,Expirationdate) >0 then 'Yes' else 'No' end as 'Is Warranty' ,ExpirationDate as 'Warranty Expiration Date',"
                               + "datediff(month,Purchasedate,Expirationdate) 'Warranty Period (months)'" +
                               " ,BillNo as [Bill Number] , Comment as [User Comment],replace(ImagePath,'~/','https://www.vcqru.com/') as [Product Image], replace(ImagePathBill,'~/','https://www.vcqru.com/') as [Bill Invoice]  " +
                               " from [dbo].[WarrentyDetails] A inner join m_code B on cast(B.Code1 as varchar(20)) +'-'+ cast(B.code2 as varchar(20)) = A.Code " +
                               " inner join pro_reg C on C.Pro_id = B.Pro_id where  C. Comp_ID = '" + Reg.Comp_ID + "' order by Purchasedate desc";
        ...
        public static DataSet FillGridDetails_PFL(Object9420 Reg)
        {
            string Qty = "SELECT  bt.Manufactured_date,bt.Expiry_date,  T_Pro.Row_ID, T_Pro.Pro_ID, T_Pro.MRP, T_Pro.Mfd_Date,(case when T_Pro.WarrantyDurationMonth > 0 then 'Yes' else 'No' end) as IsWarranty,isnull(T_Pro.WarrantyDurationMonth,0) as WarrantyDurationMonth, (CASE WHEN CONVERT(NVARCHAR,T_Pro.Exp_Date,103) is null THEN  '----' ELSE  CONVERT(NVARCHAR,T_Pro.Exp_Date,103)  END ) AS Exp_Date, T_Pro.Batch_No, T_Pro.Entry_Date," + //" T_Pro.Update_Flag_H, T_Pro.Update_Flag_E,  " +
                ...
                " Pro_Reg ON Comp_Reg.Comp_ID = Pro_Reg.Comp_ID INNER JOIN  " +
                " T_Pro ON Pro_Reg.Pro_ID = T_Pro.Pro_ID  " +
                "inner join (select distinct  Manufactured_date,Expiry_date, New_Pro_ID , New_Batch_No  from tblNewBatchAssingedDetails) bt on bt.New_Pro_ID=T_Pro.Pro_ID  and bt.New_Batch_No=T_Pro.Row_ID" +
        ...
        //public static DataSet FillGridDetails_PFL(Object9420 Reg)
        //{
        //    string Qty = "select Row_ID,Pro_ID,MRP,Mfd_Date,IsWarranty,WarrantyDurationMonth,Exp_Date,Batch_No,Entry_Date,Comp_ID,NoofCodes,SoundPath_H,SoundPath_E,Pro_Name " +
        //        "from (SELECT T_Pro.Row_ID, T_Pro.Pro_ID, T_Pro.MRP, T_Pro.Mfd_Date,(case when T_Pro.WarrantyDurationMonth > 0 then 'Yes' else 'No' end) as IsWarranty,isnull(T_Pro.WarrantyDurationMonth,0) as WarrantyDurationMonth, (CASE WHEN CONVERT(NVARCHAR,T_Pro.Exp_Date,103) is null THEN  '----' ELSE  CONVERT(NVARCHAR,T_Pro.Exp_Date,103)  END ) AS Exp_Date, T_Pro.Batch_No, T_Pro.Entry_Date," + //" T_Pro.Update_Flag_H, T_Pro.Update_Flag_E,  " +
        //        " Comp_Reg.Comp_ID,  " +
        //        " (SELECT COUNT(Row_ID) AS Expr1  " +
        //        " FROM M_Code_PFL  " +
        //        " WHERE (Batch_No =CONVERT(VARCHAR,T_Pro.Row_ID))) AS NoofCodes,   " +
        ...
        //        " Pro_Reg ON Comp_Reg.Comp_ID = Pro_Reg.Comp_ID INNER JOIN  " +
        //        " T_Pro ON Pro_Reg.Pro_ID = T_Pro.Pro_ID  " +
        //        " WHERE (Comp_Reg.Comp_ID = '" + Reg.Comp_ID + "') AND (Pro_Reg.Pro_Name LIKE '%" + Reg.Pro_Name + "%')  " + Reg.chkstr + "  )abc where NoofCodes>0 ORDER BY Row_ID DESC,Entry_Date DESC";
        //    dbCommand = database.GetSqlStringCommand(Qty); dbCommand.CommandTimeout = 500000;
        //    return database.ExecuteDataSet(dbCommand);
        //}
        ...
        public static bool FindCheckedStatus(Object9420 Reg)
        {
            ...
            if (Reg.Comp_ID == "Comp-1693")
            {
                dbCommand = database.GetSqlStringCommand("SELECT Use_Type,Pro_ID FROM  M_Code_PFL with (nolock) WHERE Code1 = '" + Reg.Received_Code1 + "' AND Code2 =  '" + Reg.Received_Code2 + "'");
                DataSet ds = database.ExecuteDataSet(dbCommand);
                if (ds.Tables[0].Rows.Count > 0)
                {
