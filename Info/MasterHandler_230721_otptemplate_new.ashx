...
public class MasterHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.QueryString["method"] == "Claim")
        {
            ...
            DataTable table = new DataTable();
            // where [gift_id]=4
            if (dtcp.Rows.Count!=0 && dtcp.Rows[0]["Comp_ID"].ToString() == "Comp-1215")
            {
                table = SQL_DB.ExecuteDataTable("select [gift_id],[Gift_name],[Gift_value] ,[Gift_desc],[Gift_image] from Claim_gift where [status]=1 order by Gift_value");
            }
            else
            {
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    if(table.Rows[i]["gift_id"].ToString()=="20")
                    {
                        ...
                        if (dtcp.Rows.Count > 0)
                        {
                            ...
                            if (dtcp.Rows[0]["Comp_ID"].ToString() == "Comp-1215")
                            {
                            ...
                            {
                                int tp = totalpoint - redeempoint - claimapply;
                                int rp = 0;
                        ...
                        {
                            //lblptmp.Text = "0";
                            //lblptmm.Text = "You are not valid for claim.";
        ...
        if(context.Request.QueryString["method"] =="dealerid")
        {
            ...
            mobile = "91" + mobile.Substring(mobile.Length - 10, 10);
            DataTable dt = new DataTable();
            dt=SQL_DB.ExecuteDataTable("select isnull(distributorid,0)  distributorid from m_consumer where mobileno='"+mobile+"'");
            if(dt.Rows.Count>0)
            {
                result = dt.Rows[0]["distributorid"].ToString();
            }
            else
            {
                result = "0";
        ...
        if (context.Request.QueryString["method"] == "SubmitClaim")
        {
            ...
            try
            {
                ...
                if (dtcp.Rows.Count > 0)
                {
                    ...
                    {
                        dtcondition = SQL_DB.ExecuteDataTable("select top 1 codition_point,condition_match from point_redeem_condition where comp_id='" + Log.Comp_id + "' and isactive=1 and selection_id=case when (select count(*) from paytmtransaction where m_consumerid='" + dt.Rows[0][1].ToString() + "' and pstatus='SUCCESS' and comp_id='Comp-1214')>0 then 2 else 1 end ");
                        if (ptforrd >= Convert.ToInt32(dtcondition.Rows[0]["codition_point"].ToString()))
                        {
                            string cn = SQL_DB.ExecuteScalar("select Comp_Name from dbo.[comp_reg] where comp_id='" + Log.Comp_id + "'").ToString();
                            ...
                            {
                                SQL_DB.ExecuteNonQuery("insert into ClaimDetails([Claim_date],[Mobileno],[Amount],[document_status],[action_date],[Isapproved],[Comp_id]) values(GETDATE(),'"+mb+"',"+ tpr.ToString() + ",1,null,0,'"+Log.Comp_id+"') ");
                            }
                            else
                            {
                                ps = ServiceLogic.Paytm_Cash(mb, tpr.ToString(), oid, null, cn);
                                if (ps.Contains("ACCEPTED"))
                                {
                                    messageobject.status = "Success";
                                    //btnptmp.Visible = false;
                                    SQL_DB.ExecuteNonQuery("insert into paytmtransaction([compid],mobileno,[pdate],[M_consumerid],Amount,pstatus,orderid) values('" + Log.Comp_id + "','" + mb + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "','" + dt.Rows[0][1].ToString() + "'," + tpr + ",'Accepted','" + oid + "')");
        ...
        else if (context.Request.QueryString["method"] == "Dashboard")
        {
            ...
            if (flg)
            {
                dtrecord = SQL_DB.ExecuteDataTable("select (SELECT case when Sum(convert(int,[Points])) is null then 0 else Sum(convert(int,[Points])) end FROM [BLoyaltyPointsEarned] where [M_Consumerid]= "+consumerid+"),isnull((case when Sum(convert(decimal(10,2),cash)) is null then 0 else Sum(convert(decimal(10,2),cash)) end)+((case when Sum(convert(decimal(10,2),cash)) is null then 0 else Sum(convert(decimal(10,2),Cash)) end)*(select top 1 calculation_value from loyalty_calculation where comp_id=max(mcon.compid))/100),0) ,Count([Gift]) from BLoyaltyPointsEarned bl inner join BuiltLoyaltyMCodeCheck blc on blc.Pkid=bl.BuildLoyaltyOrReferralMCodeCheckid inner join M_Consumer_M_Code mcon on  mcon.m_consumer_mcodeid=blc.m_consumer_mcodeid inner join m_code code on code.row_id=mcon.M_Codeid inner join Pro_Enq pe on pe.received_code1=code.code1 and pe.received_code2=code.code2 inner join enq_dealerid dealer on dealer.enq_id=pe.row_id inner join M_Consumer cons on cons.distributorID=dealer.dealerid where cons.M_Consumerid="+consumerid+" and pe.is_success=1");
            }
            else
            {
            ...
            try
            {
                ...
                if(messageobject.message==""||messageobject.message==null)
                {
                    ...
                    if (mobile != "")
                        ...
                        {
                            if (app.CompanyName == "JOHNSON PAINTS CO." || app.CompanyName == "SKYDECOR LAMINATES PRIVATE LIMITED"||app.CompanyName == "COATS BATH FITTINGS")
                                result = appchkgenuenity(codeone, codetwo, paintermobile, app.fields, mode,dealerid,paintername,mobile);
                            else
                                result = appchkgenuenity(codeone, codetwo, mobile, app.fields, mode);
                        }
                }
    ...
    public static string appchkgenuenity(string code1, string code2, string mobile, string fields, string mode,string dealerid="",string paintername="",string dealermobile="")
    ...
    public static string chkgenuenity(string code1, string code2, string mobile, string email, string ReferralCodeFromUser, string city, string name,string dealerid="",string designation="",string dealermobile="")
    // public static string chkgenuenity(string code1, string code2, string mobile)
    {
        //return "testing <br/>  Congratulation's !!! <b>You</b> have won gift coupon of";
        string result = "";
    ...
    public void UpdateClaimDetailsVendor(string comment, string approveStatus, string id)
    {
        SQL_DB.ExecuteNonQuery("update [dbo].[ClaimDetails] set Isapproved='" + approveStatus + "',[vendor_comment]='" + comment + "', action_date=getdate() where Row_id=" + id + "");
    }
