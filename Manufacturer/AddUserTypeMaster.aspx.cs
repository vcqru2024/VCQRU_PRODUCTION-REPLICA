...
public partial class Manufacturer_AddUserTypeMaster : System.Web.UI.Page
{
    public int Flag = 0, index = 0, sno = 1;

    private string _UTid_Prop;
    public int UTid_Prop
    {
        get {
            if(ViewState["_UTid_Prop"] == null)
            {
                return 0;
            }
            else
            {
                return (int)ViewState["_UTid_Prop"];
            }            
        }
        set { ViewState["_UTid_Prop"] = value; }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                UTid_Prop = Convert.ToInt32(Request.QueryString["id"]);
                EditMode();
            }
    ...
    public void EditMode()
    {
        ...
        DataTable dt = SQL_DB.ExecuteDataTable("Select * from UserType_TrackTrace where TTUserTypeid =" + UTid_Prop);
        if (dt.Rows.Count > 0)
        {

            txtName.Text = dt.Rows[0]["type"].ToString();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(txtName.Text))
        {
            if (UTid_Prop >0)
            {
                SQL_DB.ExecuteNonQuery("Update UserType_TrackTrace set type = '" + txtName.Text.Replace("'", "") + "' where TTUserTypeid=" + UTid_Prop);
            }
            else
            {
                SQL_DB.ExecuteNonQuery("insert into UserType_TrackTrace(type,compid) values('" + txtName.Text.Replace("'", "") + "','" + ProjectSession.strCompanyid + "')");
            }
        ...
        Response.Redirect("FrmUserTypeMaster.aspx?edit=s");
    }
    
    protected void btnReset_Click(object sender, EventArgs e)
    {
        Response.Redirect("FrmUserTypeMaster.aspx");
