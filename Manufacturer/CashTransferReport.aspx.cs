...
public partial class Manufacturer_CashTransferReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["CompanyId"] == null)
            Response.Redirect("index.html?Page=CashTransferReport.aspx");
        else
        {
        ...
        if (!Page.IsPostBack)
        {
            if (!string.IsNullOrEmpty(Session["CompanyId"].ToString()))
                FillGrdLoyaltyAwardsMidas(Session["CompanyId"].ToString());
    ...
    private void FillGrdLoyaltyAwardsMidas(string email)
    {
        //if (Session["CompanyId"].ToString() != "Comp-1262")
        //{
        ...
        DataSet ds = Warranty.GetClaimDetailVendorMidas(email);
    ...
    protected void btnDownloadExcel_Click(object sender, EventArgs e)
    {
        try
        {
            DataSet ds = Warranty.GetClaimDetailVendor_downloadMidas(Session["CompanyId"].ToString());
            XLWorkbook wb = new XLWorkbook();
            wb.Worksheets.Add(ds.Tables[0], "Cash Transfer Report");
            ...
            Response.AddHeader("content-disposition", "attachment;    filename=Cash_Transfer_Report.xlsx");
