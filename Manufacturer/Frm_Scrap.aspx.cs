...
public partial class Frm_Scrap : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        for (int K = 0; K < Convert.ToInt32(lblC.Text); K++)
        {
            ...
            if (Arr.Length > 0)
            {
                for (int p = 0; p < Arr.Length; p++)
                {
                    ...
                    {
                        if (batch != "0")
                            updateqry = "UPDATE [M_Code] SET  [Batch_No] = " + batch + "  ,[ScrapeFlag] = 0 WHERE Pro_ID='" + ddlProduct.SelectedValue + "'  and Print_Status=1 and Series_Order=" + series_order + "  and Series_Serial=" + series_serial + " AND [Batch_No] IS NULL AND [ScrapeFlag] = 1 ";
                        else
                            updateqry = "UPDATE [M_Code] SET  [ScrapeFlag] = 0 WHERE Pro_ID='" + ddlProduct.SelectedValue + "'  and Print_Status=1 and Series_Order=" + series_order + "  and Series_Serial=" + series_serial + " AND [Batch_No] IS NULL ";
        ...
        if (Request.Form["chkselect"] != null)
        {
            ...
            for (int i = 0; i < Arr.Length; i++)
            {
                ...
                string Qry = "SELECT isnull([Batch_No],0) FROM [M_Code] WHERE Pro_Id='" + pro + "' and [Series_Order]=" + series_order + " and [Series_Serial]=" + series_serial;// and ScrapeFlag=0
                string batch = SQL_DB.ExecuteScalar(Qry).ToString();
                updateqry = "UPDATE [M_Code] SET [Batch_No] = null ,[ScrapeFlag] = 1 WHERE Pro_ID='" + ddlProduct.SelectedValue + "'  and Print_Status=1 and Series_Order=" + series_order + "  and Series_Serial=" + series_serial + "";
                SQL_DB.ExecuteNonQuery(updateqry);
