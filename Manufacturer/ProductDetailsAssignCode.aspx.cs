...
public partial class Manufacturer_ProductDetailsAssignCode : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["CompanyId"] == null)
            Response.Redirect("/Login.aspx?Page=ProductDetails");
        else
        {
    ...
    private void filldata()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
            ...
            if (Session["UserRole_id"] != null)
            {
                var filteredRows = ds.Tables[0].AsEnumerable()
                    .Where(row => !string.IsNullOrEmpty(row.Field<string>("Pro_ID")) && Userasignproduct.Contains(row.Field<string>("Pro_ID")));

                DataTable filteredTable = filteredRows.Any() ? filteredRows.CopyToDataTable() : ds.Tables[0].Clone(); // Clone structure if no rows match
                if (filteredTable.Rows.Count == 0)
                {
                    GrdProductMaster.DataSource = null;
                    GrdProductMaster.DataBind();
                    lblcount.Text = "0";
                    DataTable emptyTable = ds.Tables[0].Clone();
                    GrdProductMaster.DataSource = emptyTable;
                    GrdProductMaster.DataBind();
                    lblcount.Text = "No records found";
                }
                else
                {
                    var filteredRowsnew = ds.Tables[0].AsEnumerable()
                     .Where(row => Userasignproduct.Contains(row.Field<string>("Pro_ID")));
                    DataTable filteredTablenew = filteredRowsnew.CopyToDataTable();
        ...
        if (Session["CompanyId"].ToString() == "Comp-1624")
        {
            ...
            List<string> columnsToHide = new List<string> { "Batch No", "MRP", "Mfd_Date", "Exp_Date" };

            foreach (DataControlField column in GrdProductMaster.Columns)
            {
                if (column is TemplateField && columnsToHide.Contains(((TemplateField)column).HeaderText))
                {
    ...
    protected void GrdProductMaster_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("addSoftCodeToProductDetails.aspx?id=" + e.CommandArgument.ToString());
        ...
        #region New code for download excel csv pdf
        if (e.CommandName == "DownLodHplExcel" || e.CommandName == "DownLodHplCSV" || e.CommandName == "DownLodHplPDF")
        {
            Object9420 obj = new Object9420();
            obj.Row_ID = e.CommandArgument.ToString();
            DataSet ds = new DataSet();
            ...
            int series_From = Convert.ToInt32(Arr1[2].ToString());
            int series_To = Convert.ToInt32(Arr2[2].ToString());
            string FileName = ds.Tables[0].Rows[0]["Batch_No"].ToString() + "_" + Convert.ToDateTime(ds.Tables[0].Rows[0]["Entry_Date"]).ToString("dd-MM-yyyy hh:mm:ss tt");
            string Qry = "SELECT row_number() OVER (ORDER BY Excel.Series_Name) AS SNO,* from (SELECT Pro_ID +'-'+ (case when len(convert(nvarchar,Series_Order)) = 1 then '0'+convert(nvarchar,Series_Order) else convert(nvarchar,Series_Order) end)+'-'+  " +
                "(case when len(convert(nvarchar,Series_Serial)) = 1 then '000'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 2 then '00'+convert(nvarchar,Series_Serial) when len(convert(nvarchar,Series_Serial)) = 3 then '0'+convert(nvarchar,Series_Serial) else convert(nvarchar,Series_Serial) end) AS Series_Name ,Code1,Code2,'https://uat.vcqru.com/codeCheck.aspx?codeone=' + CAST(Code1 AS NVARCHAR(100)) + '-' + CAST(Code2 AS NVARCHAR(100)) AS URL FROM  M_Code " +
                "WHERE Pro_ID='" + Pro_ID + "' AND Batch_No='" + ds.Tables[0].Rows[0]["Row_ID"].ToString() + "' and print_status = 1 and ScrapeFlag is  null AND (Series_Order = " + series_order + ") AND (Series_Serial >= " + series_From + ") AND (Series_Serial <= " + series_To + ") ) as Excel";

            switch (e.CommandName)
            {
                case "DownLodHplExcel":
                    downloadHpl(Qry, FileName, Session["Comp_Name"].ToString(), ds.Tables[0].Rows[0]["Pro_Name"].ToString(), ds.Tables[0].Rows[0]["Batch_No"].ToString(), Convert.ToDateTime(ds.Tables[0].Rows[0]["Entry_Date"].ToString()).ToString("dd/MM/yyyy"));
                    break;
                case "DownLodHplCSV":
                    downloadCsv(Qry, FileName);
                    break;
                case "DownLodHplPDF":
                    downloadPdf(Qry, FileName);
                    break;
            }
        }
        #endregion
    ...
    private void downloadHpl(string Qry, string FileName, string Comp_Name, string Pro_Name, string Batch_No, string Pasted_Date)
    {
    ...
    private void downloadCsv(string query, string fileName)
    {
        DataSet ds = SQL_DB.ExecuteDataSet(query);
    ...
    public void downloadPdf(string query, string fileName)
    {
        DataSet ds = SQL_DB.ExecuteDataSet(query);
        using (var ms = new MemoryStream())
        {
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                string url1 = row["URL"].ToString();
                Bitmap qrCodeImage1 = GenerateQRCode(url1, 1);
                Bitmap qrCodeImage2 = GenerateQRCode(url1, 1);
                Bitmap qrCodeImage3 = GenerateQRCode(url1, 1);
                iTextSharp.text.Image pdfImage1 = ConvertBitmapToPdfImage(qrCodeImage1);
                iTextSharp.text.Image pdfImage2 = ConvertBitmapToPdfImage(qrCodeImage2);
                iTextSharp.text.Image pdfImage3 = ConvertBitmapToPdfImage(qrCodeImage3);
                PdfPTable mainTable = new PdfPTable(3);
                mainTable.WidthPercentage = 25;
                PdfPTable nestedTable1 = CreateNestedTable(row.ItemArray[1].ToString() + "\n" + row.ItemArray[2].ToString() + "-" + row.ItemArray[3].ToString(), pdfImage1);
                PdfPTable nestedTable2 = CreateNestedTable(row.ItemArray[1].ToString() + "\n" + row.ItemArray[2].ToString() + "-" + row.ItemArray[3].ToString(), pdfImage2);
                PdfPTable nestedTable3 = CreateNestedTable(row.ItemArray[1].ToString() + "\n" + row.ItemArray[2].ToString() + "-" + row.ItemArray[3].ToString(), pdfImage3);
                mainTable.AddCell(new PdfPCell(nestedTable1) { Border = PdfPCell.NO_BORDER });
                mainTable.AddCell(new PdfPCell(nestedTable2) { Border = PdfPCell.NO_BORDER });
                mainTable.AddCell(new PdfPCell(nestedTable3) { Border = PdfPCell.NO_BORDER });
    ...
    private PdfPTable CreateNestedTable(string text, iTextSharp.text.Image qrImage)
    {
        PdfPTable nestedTable = new PdfPTable(1);
        nestedTable.WidthPercentage = 100;
        PdfPCell textCell = CreateRotatedTextCell(text);
    ...
    private iTextSharp.text.Image ConvertBitmapToPdfImage(Bitmap bitmap)
    {
        using (var imageStream = new MemoryStream())
        {
            ...
            return iTextSharp.text.Image.GetInstance(imageStream.ToArray());
        }
    }
    private PdfPCell CreateRotatedTextCell(string text)
    {
    ...
    {
        Response.Redirect("addSoftCodeToProductDetails.aspx");
    }
}