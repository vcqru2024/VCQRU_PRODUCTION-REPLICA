...
public partial class Manufacturer_SearchCodeDetails : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["User_Type"] == null)
            Response.Redirect("../Info/Login.aspx?Page=Frm_Scrap.aspx");
        else
        {
    ...
    private void ScrapEntry(string Pro_Id, string BatchNo)
    {
        try
        {
            ...
            if (txtcode.Text != "")
            {
                string NewSelectedBatch = "From "+ddlProduct.SelectedItem.Value+"-00-0000 To "+ ddlProduct.SelectedItem.Value + "-00-0000";
                if (lblseriesdetails.Text!="")
                NewSelectedBatch = lblseriesdetails.Text;
            ...
            if (hidden1.Value != null)//(Request.Form["chkselect"] != null)
            {
                ...
                for (int i = 0; i < Arr.Length; i++)
                {
                    ...
                    //string smcodeProID = SQL_DB.ExecuteScalar("select top(1) Pro_ID from M_code where Pro_ID = '"+ Newpro + "'").ToString();
                        ...
                        string SelQury = "select p.Pro_ID,m.Row_ID,m.Pro_ID +'-'+convert(varchar,(case when len(convert(nvarchar,m.Series_Order)) = 1 then" +
                            ...
                            "then '00'+ convert(nvarchar,m.Series_Serial) when len(convert(nvarchar,m.Series_Serial)) = 3 then '0'+ convert(nvarchar,m.Series_Serial) " +
                            "else convert(nvarchar,m.Series_Serial) end)) as SerialCode,m.Batch_No " +
                            "from M_Code m inner join Pro_Reg p on p.Pro_ID = m.Pro_ID " +
                            "inner join M_ServiceSubscription s on s.Comp_ID = p.Comp_ID and s.Pro_ID = m.Pro_ID " +
                            "inner join M_ServiceSubscriptionTrans st on st.Subscribe_Id = s.Subscribe_Id and st.IsActive=1 " +
                    ...
                    {
                        string Series_Limit = SelectedBatch[0] + "To" + NewSerialCode;
                        /*string strSsidCodegen = SQL_DB.ExecuteScalar("select top(1) PrStart from Code_Gen where PrPrefix = 'SSI' and Prfor = 'Subscription'").ToString();
                        string newSubscribe_Id = "SSI" + strSsidCodegen;
                       string OldSubscribe_Id = SQL_DB.ExecuteScalar("select top(1) Subscribe_Id from M_ServiceSubscription where Pro_ID='" + Oldpro + "' ").ToString();

                        Random rnd = new Random();
                       string batch = rnd.Next().ToString().Substring(0, 5);

                        string strServiceSubsEntry = "insert into M_ServiceSubscription (Subscribe_Id,Service_ID,Comp_ID,Pro_ID,Plan_ID,PlanName,PlanMasterPeriod,PlanSalePeriod,PlanMasterPrice,PlanSalePrice,DateFrom,DateTo,EntryDate,IsActive,IsDelete,IsAdminVerify,TransType,start_order,start_series,end_order,end_series) " +
                    "select top(1) '" + newSubscribe_Id + "',Service_ID,Comp_ID,'" + Newpro + "',Plan_ID,PlanName,PlanMasterPeriod,PlanSalePeriod,PlanMasterPrice,PlanSalePrice,DateFrom,DateTo,EntryDate,IsActive,IsDelete,IsAdminVerify,TransType,start_order,start_series,end_order,end_series from M_ServiceSubscription " +
                    "where Subscribe_Id = '" + OldSubscribe_Id + "' and Comp_ID = '" + Comp_ID + "' and Pro_ID = '" + Oldpro + "' ";

                        string strServSubTrans = "insert into M_ServiceSubscriptionTrans (Subscribe_Id,Points,IsCashConvert,IsCash,DateFrom,DateTo,Entry_Date,Update_Flag_H,Update_Flag_E,Comments,Frequency,IsActive,IsDelete,IsDraw,IsReferral,DrawDate,WarrantyPeriod,AmtType,Minval,Maxval,totalamont) " +
                            "select top(1) '" + newSubscribe_Id + "',Points,IsCashConvert,IsCash,DateFrom,DateTo,Entry_Date,Update_Flag_H,Update_Flag_E,Comments,Frequency,IsActive,IsDelete,IsDraw,IsReferral,DrawDate,WarrantyPeriod,AmtType,Minval,Maxval,totalamont from M_ServiceSubscriptionTrans " +
                            "where Subscribe_Id='" + OldSubscribe_Id + "' ";

                        string strBatcNo = "insert into T_Pro (Pro_ID, MRP, Mfd_Date, Exp_Date, Batch_No, Entry_Date, Update_Flag_H, Update_Flag_E, Series_Limit,Comments, IsWarranty, WarrantyDurationMonth)" +
                            "select top(1) '" + Newpro + "',MRP,Mfd_Date, Exp_Date, '"+ batch + "', Entry_Date, Update_Flag_H, Update_Flag_E, '"+ Series_Limit + "','', NULL,'0'" +
                            "from T_Pro where pro_ID = '"+Oldpro+"' and series_limit = '"+Oldseries_serial+"'";

                        SQL_DB.ExecuteNonQuery(strServiceSubsEntry);
                        SQL_DB.ExecuteNonQuery(strServSubTrans);

                        SQL_DB.ExecuteNonQuery("update Code_Gen set PrStart=PrStart+1 where PrPrefix = 'SSI' and Prfor = 'Subscription'");
                        SQL_DB.ExecuteNonQuery("update Code_Gen set PrStart=PrStart+1 where PrPrefix = 'LTNO' and Prfor = 'LabelTracking'");


                        */

                        string grpupdateqry = "UPDATE [M_Code] SET Pro_ID='"+ Newpro +"',Batch_No='"+ BatchNo +"',[Series_Order]="+ Newseries_order+", [Series_Serial]="+ Newseries_serial +" WHERE  Pro_ID='" + Oldpro + "' AND Series_Order =" + Oldseries_order + " AND Series_Serial = " + Oldseries_serial + " ";
                        SQL_DB.ExecuteNonQuery(grpupdateqry);
    ...
    private void fillgrid()
        ...
        string qry = "select p.Pro_Name,cast(m.Code1 as varchar(5)) + cast(m.Code2 as varchar(8)) Completedcode," +
            ...
            "from M_Code m inner join Pro_Reg p on p.Pro_ID = m.Pro_ID inner join M_ServiceSubscription s on s.Comp_ID = p.Comp_ID " +
        ...
        else if (txtscrapfrom.Text != "" && txtscrapto.Text != "")
        {
            ...
            if (Series_Order == Series_Order1 && pro == pro1)
            {
                ...
                str += " AND m.Pro_ID='" + pro1 + "' AND Use_Count is null";
            }
            else
            {
