...
public partial class Manufacturer_StateWiseProductReport : System.Web.UI.Page
{
    ...
    public void Bindgrid(string datefrom, string dateto)
    {
        ...
        con.Open();
        SqlCommand cmd = new SqlCommand("USP_GetStateWiseProduct_HPL", con);
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.AddWithValue("@companyid", Session["CompanyId"].ToString());
        cmd.Parameters.AddWithValue("@start_date", datefrom);
        cmd.Parameters.AddWithValue("@enq_date", dateto);
    ...
    protected void btn_download_Click(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@enq_date", datetostr);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                con.Close();
                XLWorkbook wb = new XLWorkbook();
                wb.Worksheets.Add(dt, "StateWiseProductReport");
                MemoryStream stream = new MemoryStream();
                wb.SaveAs(stream);
                Response.Clear();
                Response.ContentType = "application/force-download";
                Response.AddHeader("content-disposition", "attachment;    filename=StateWiseProductReport.xlsx");
