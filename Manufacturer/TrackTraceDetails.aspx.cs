...
public partial class TrackTraceDetails : System.Web.UI.Page
{
    ...
    public void BindListboxproductionunit()
    {
        DataTable dt = SQL_DB.ExecuteDataTable("select * from productionunit where compid='" + ProjectSession.strCompanyid + "'");
        if (dt.Rows.Count > 0)
        {
            lstProdUnit.DataSource = dt;
            lstProdUnit.DataTextField = "name";
            lstProdUnit.DataValueField = "ProductionUnitID";
            lstProdUnit.DataBind();
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (FileUploadDistributor.HasFile)
        {
            ...
            UploadExcel(strFilename, "2");//"distributor"
        }
    ...
    {
        if (!string.IsNullOrEmpty(txtProductionInit.Text))
        {
            lstProdUnit.Items.Add(new ListItem(txtProductionInit.Text, txtProductionInit.Text));
        }
        txtProductionInit.Text = "";
    }

    protected void Button1_Click(object sender, EventArgs e)
    {
        //ListBox.SelectedObjectCollection selectedItems = new ListBox.SelectedObjectCollection(lstClientes);
        //selectedItems = lstProdUnit.SelectedItem;

        if (lstProdUnit.SelectedIndex != -1)
        {
            for (int i = 0; i < lstProdUnit.Items.Count; i++)
            {
                if (lstProdUnit.SelectedItem.Text == lstProdUnit.Items[i].Text)
                {
                    lstProdUnit.Items.Remove(lstProdUnit.Items[i].Text);
                    BindListboxproductionunit();
                }
            }
        }

        //lstProdUnit.Items.RemoveAt();
      
        //ArrayList vSelectedItems = new ArrayList(lstProdUnit.SelectedItem);
        //lstProdUnit.Items.Remove(lstProdUnit.SelectedItem.Text);
        //for (int i = 0; i < lstProdUnit.SelectedItems.Count; i++)
        //{

        //    //lstProdUnit.SelectedItem
        //    if (lstProdUnit.Items[i].Selected)
        //    {
    ...
    protected void btnChannels_Click(object sender, EventArgs e)
    {
        ...
        {
            lstChannels.Items.Add(new ListItem(txtChannels.Text, txtChannels.Text));
        }
        txtChannels.Text = "";
    }
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        if (lstChannels.SelectedIndex != -1)
        {
            ...
            {
                if (lstChannels.SelectedItem.Text == lstChannels.Items[i].Text)
                {
                    lstChannels.Items.Remove(lstChannels.Items[i].Text);
                    BindListboxchannels();
                }
            }
        }
