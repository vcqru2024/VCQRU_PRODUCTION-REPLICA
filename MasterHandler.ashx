...
public class MasterHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public string GetConsumerDetailsByMobileNo(string MobileNo, string codeone = "", string codetwo = "")
    {
        string result = "NA";
        ...
        }else if (codeone == "" && codetwo == "" && MobileNo !="")
        { // added by dipak tiwari for max paint
            ...
            result = ConsumerName + "~" + Inox_User_Type + "~" + City + "~" + state + "~" + SellerName + "~" + Other_Role + "~" + PinCode + "~" + Email + "~" + BankName + "~" + AccountNumber + "~" + IfscCode + "~" + UPIId + "~" + BranchName + "~" + AccountHolderName + "~" + Address+ "~" + department+ "~" + organization;
        }
        else
            #endregion
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.QueryString["method"] == "ConsumerDetailsByMobileNo")
        {
            ...
            if (codeone.Trim().Length == 5 && codetwo.Trim().Length == 8)
                ...
                #endregion
                //  string result = "";
                result = GetConsumerDetailsByMobileNo(mobile);
            context.Response.Write(result);
        }
        ...
        else if (context.Request.QueryString["method"] == "updateclaimVendor")
        {
            ...
            if ((company_name == "Lubigen Pvt Ltd") && approveStatus == "1")
            {
                ...
                if (txtUpiId != "")
                {
                    ...
                    if (company_name == "Lubigen Pvt Ltd")
                    {
                        postData = "{\"Mobile\":\"" + dt.Rows[0]["MobileNo"].ToString() + "\",\"ProductId\":\"138\",\"Productvalue\":\"250\",\"ServiceId\":\"SRV1029\",\"Amount\":\"" + ptforrd + "\",\"UPIID\":\"" + txtUpiId + "\"}";
                    }
                    else
                    {
                    ...
                    //string postData = "{\"Mobile\":\"" + dt.Rows[0]["MobileNo"].ToString() + "\",\"ProductId\":\"" + id + "\",\"Productvalue\":\""+ptforrd+"\",\"ServiceId\":\"SRV1029\",\"Amount\":\"" + ptforrd + "\",\"UPIID\":\"" + txtUpiId + "\"}";
    ...
    public static string chkgenuenity(string code1, string code2, string mobile, string email, string Role_ID, string State, string ReferralCodeFromUser, string city, string name, string dealerid = "", string designation = "", string dealermobile = "", string village = "", string district = "", string state = "", string compname = "", string country = "", string bookname = "", string bookShop = "", string ccenter = "", string Address = "", string SellerName = "", string CompID = "", string Other_Role = "", string PinCode = "", string BankName = "", string AccountNumber = "", string IfscCode = "", string AccountHolderName = "", string BranchName = "", string virtualPath = "", string TNC = "", string UPI = "", string Shopname = "", string Gender = "", string Age = "", string RefralCode = "", string pancard_number = "", string IsVerified = "", string Latitude = "", string Longitude = "", string aadhar_number = "")
    // public static string chkgenuenity(string code1, string code2, string mobile)
    {
        //return "testing <br/>  Congratulation's !!! <b>You</b> have won gift coupon of";
        string result = "";
        ...
        DataSet dcs = SQL_DB.ExecuteDataSet("SELECT [User_ID],[MobileNo],[Email],[ConsumerName],village,district,[state] FROM [M_Consumer] where right([MobileNo],10) = '" + MobileNo.Substring(MobileNo.Length - 10) + "'");
        if (dcs.Tables[0].Rows.Count > 0)
        {
            // below line is commented by shweta
            //if (dcs.Tables[0].Rows[0]["Email"].ToString() != mobile.Trim().Replace("'", "''"))
        ...
        if (ConsumerId == "")
        {
            ...
            if (dtnew.Rows.Count > 0)
            {
                ...
                if ((compname == "Oltimo Lubes" || compname == "CHERYL CHEMICAL AND POLYMERS" ||compname == "DURGA COLOUR AND CHEM.P LTD." || compname == "OCI Wires and Cables" || compname == "SAMSOIL PETROLEUM INDIA LTD" || compname == "SRI ANANTHA PADMANABHA SWAMY ENTERPRISES") && (designation !="" || Role_ID != "" ||  UPI != "" || aadhar_number != "" || pancard_number != "" || PinCode != "" || city != "" || state != ""))
                {
            ...
            if (compname == "CHERYL CHEMICAL AND POLYMERS" || compname == "DURGA COLOUR AND CHEM.P LTD." ||compname == "Shree Durga Traders" || compname == "NV INFOTECH DOMAIN PRIVATE LIMITED" || compname == "BLUEGEM PAINTS" || compname == "The Kolorado  Paints" || compname == "Oltimo Lubes" || compname == "HERBAL AYURVEDA AND RESEARCH CENTER" || compname == "SAMSOIL PETROLEUM INDIA LTD" && (AccountNumber != "" || IfscCode != "" || BankName != "" || BranchName != ""))
            {
        ...
        else if (compname == "DURGA COLOUR AND CHEM.P LTD." || compname == "Shree Durga Traders" || compname == "NV INFOTECH DOMAIN PRIVATE LIMITED" || compname == "BLUEGEM PAINTS" || compname == "The Kolorado  Paints" || compname == "HERBAL AYURVEDA AND RESEARCH CENTER" || compname == "SAMSOIL PETROLEUM INDIA LTD" && (AccountNumber != "" || IfscCode != "" || BankName != "" || BranchName != ""))
        {
            string M_Consumerid = SQL_DB.ExecuteScalar("select [M_Consumerid] FROM [M_Consumer] where right([MobileNo],10) = '" + MobileNo.Substring(MobileNo.Length - 10) + "'").ToString();
            Object9420 Log = new Object9420();
        ...
        else if (compname == "CHERYL CHEMICAL AND POLYMERS" || compname == "DURGA COLOUR AND CHEM.P LTD.")//this condition added for max paints to update existing consumer data
        {
            Random rnd = new Random();
        ...
        else if ((compname == "SRI ANANTHA PADMANABHA SWAMY ENTERPRISES" )&& (UPI != "" || aadhar_number != "" || pancard_number != "" || PinCode != "" || city != "" || state != ""))
        {
            string User_ID = string.Empty;
            DataTable dtnew = SQL_DB.ExecuteDataTable("select * FROM [M_Consumer] where right([MobileNo],10) = '" + MobileNo.Substring(MobileNo.Length - 10) + "'");
            if (dtnew.Rows.Count > 0)
        ...
        else if (compname == "EVERCROP AGRO SCIENCE" && (consumer_name == "" || dvillage == "" || ddistrict == "" || dstate == ""))
        {
            #region Registration
            Random rnd = new Random();
            User_Details Log = new User_Details();
        ...
        else if (compname == "CHERYL CHEMICAL AND POLYMERS" || compname == "OCI Wires and Cables"  ||compname == "TRUE NUTRITION PERFORMANCE PRIVATE LIMITED"  ||compname == "FITSIQUE"  ||compname == "TECHNICAL MINDS PRIVATE LIMITED" || compname == "ORBIT ELECTRODOMESTICS INDIA")
        {
            #region Registration
        ...
        if (ds.Tables[0].Rows.Count == 0)
        {
            ...
            else
            {
                ...
                if (compname == "Paras cosmetics private limiteds" || compname == "Ram Gopal and Sons")
                {
                    ...
                    result = "The code entered is invalid. Please check and try again.";
                if (compname == "SRI ANANTHA PADMANABHA SWAMY ENTERPRISES")
                {
                    result = "The code entered is invalid. Please check and try again";
                }
                        ...
                        else
                        {
                            ...
                            if (CompanyName1 != "" && CompanyName1 == "VR KABLE INDIA PRIVATE LIMITED")
                            {
                                ...
                                if (KycCheck == "1" || KycCheck == "0")
                                {
                                    ...
                                    else if (VrUser_Type == "4" && ( proIDD == "AM46" || proIDD == "AH86" || proIDD == "AH87" || proIDD == "AH88" || proIDD == "AH89" || proIDD == "AH90" || proIDD == "AI23" || proIDD == "AJ39" || proIDD == "AJ40" || proIDD == "AK01" || proIDD == "AL66" || proIDD == "AL67" || proIDD == "AL68" || proIDD == "AL93" || proIDD == "AL92")) // electrician
                                    {
                                    }
                                    else
                                    {
