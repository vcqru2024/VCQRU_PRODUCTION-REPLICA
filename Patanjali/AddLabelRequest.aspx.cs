...
public partial class Patanjali_AddLabelRequest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["CompanyId"] == null)
            Response.Redirect("../patanjali/loginpfl.aspx?Page=AddLabelRequest.aspx");
        else
        {
    ...
    protected void btnRequestSend_Click(object sender, EventArgs e)
    {
        ...
        if (Convert.ToInt32(txtNoofLabel.Text) == 0)
        {
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "Invalid request", "showAlert('Error', 'Please enter requested  No. of print Labels !', 'error');", true);
            return;
            
        }
        else if (ddlprotype.SelectedValue == "--Select--")
        {
        //    DivNewMsg.Visible = true;
        //    DivNewMsg.Attributes.Add("class", "alert_boxes_pink");
        //    LabelRequestmsg.Text = "Please select a product to make a request!";
        //    // ModalPopupExtenderRequest.Show();
        //    string script = @"setTimeout(function(){document.getElementById('" + DivNewMsg.ClientID + "').style.display='none';},5000);";
        //    Page.ClientScript.RegisterStartupScript(this.GetType(), "somekey", script, true);
        //    return;

            ScriptManager.RegisterStartupScript(this, GetType(), "Invalid request", "showAlert('Error', 'Please select a product to make a request!', 'error');", true);
            return;
        ...
        else if (!function9420.chkVerifyDoc(Reg))
        {
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "Invalid request", "showAlert('Error', 'Your product documents or sound file are not verified by Admin!', 'error');", true);
            return;
        }
        else
        {
