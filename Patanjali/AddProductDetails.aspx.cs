...
public partial class Patanjali_AddProductDetails : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["CompanyId"] == null)
            Response.Redirect("../patanjali/loginpfl.aspx?Page=AddProductDetails.aspx");
        else
        {
    ...
    private void filldata()
    {
        ...
        ddlProduct.SelectedValue = obj.Pro_ID.Trim().ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        #region Stattic value which removed from UI as per client suggestion
        txtBatchNo.Text = "6666";
        txtMRP.Text = "1";
        txtExp_Date.Text = System.DateTime.Now.AddMonths(60).ToString("yyyy-MM-dd");
        txtMfd_Date.Text = System.DateTime.Now.ToString("yyyy-MM-dd");
  ...
  if(txtBatchNo.Text.Trim() =="")
            {
                LogManager.WriteExe("Error Batch no reqiore!");
                
                LblMsg.Text = "Batch no. required!  ";
                return;
            }
            if (txtMRP.Text == "")
            {
                LogManager.WriteExe("Error MRP reqiore!");

                LblMsg.Text = "MRP required!  ";
                return;
            }
           
            if (txtExp_Date.Text == "")
            {
                LogManager.WriteExe("Error exp date reqiore!");

                LblMsg.Text = "Exp date required!  ";
                return;
            }
            ...
            */
            ...
            if (Business9420.function9420.GetRowForPrintCodes(obj) > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if(Session["CompanyId"].ToString()== "Comp-1693")
