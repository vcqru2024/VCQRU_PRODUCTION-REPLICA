...
public partial class Patanjali_CodeVerificationReport : System.Web.UI.Page
{
    ...
    public DataSet getdataset()
    {
        ...
        if (Page.IsValid)
        {
            try
            {
                 dtst = FillcodeverificationPFL("Comp-1311", product, Convert.ToDateTime(txtTodate.Value), Convert.ToDateTime(txtfromdate.Value), Reg.statusstr, Reg.Remarks, Mobile.Text, state, Reg.Service_ID);
            }
            catch (Exception ex)
            {
                Console.WriteLine("An error occurred: " + ex.Message);
    ...
    protected void ddlservice_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        FillProducts("Comp-1311");
    }
    private void fillservice()
    {
        DataSet ds = SQL_DB.ExecuteDataSet("select distinct sb.service_id,serv.servicename from M_ServiceSubscription (nolock) sb left join m_service (nolock) serv on sb.Service_ID=serv.Service_ID where sb.Comp_ID='Comp-1311'");
    ...
    {
        DataSet ds = ExecuteNonQueryAndDatatable.filllocation("Comp-1311", service);
