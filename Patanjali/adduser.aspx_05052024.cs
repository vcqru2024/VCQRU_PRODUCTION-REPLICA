    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if(btnSubmit.Text== "Update")
        {
            ...
            if (dtget.Rows.Count > 0)
            {
                ...
                if (Session["UserName"] != null)
                {
                    db.ExceptionLogs("User Role Upated By", Session["UserName"].ToString(), "U");
                }
                else
                {
                    db.ExceptionLogs("User Role Upated By", Session["Comp_Name"].ToString(), "U");
                }
                #endregion
        ...
        else
        {
            ...
            bool isValidName = IsValidUserName(Name);
            if (!isValidName)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "Invalid", "showAlert('Error', 'Please Enter Valid User Name', 'error');", true);
                return;
            }
            if (!isValidEmail)
            {
            ...
            if (!string.IsNullOrEmpty(Name) && !string.IsNullOrEmpty(Email) && !string.IsNullOrEmpty(Mobile))
            {
                ...
                if (dt.Rows[0][0].ToString() != "Entered Mobile Number Already Used" && dt.Rows[0][0].ToString() != "Entered Email Id Already Used")
                {
                    ...
                    if (dtUser.Rows.Count > 0)
                    {
                        ...
                        #endregion
                       #region  Loop for Insert Transaction Menu Selection Commented Now
                        //for (int i = 0; i < ChkTransaction.Items.Count; i++)
                        //{
                        //    if (ChkTransaction.Items[i].Selected)
                        //    {
                        ...
                        if (Session["UserName"] != null)
                        {
                            db.ExceptionLogs("User Added By", Session["UserName"].ToString(), "I");
                        }
                        else
                        {
                            db.ExceptionLogs("User Added By", Session["Comp_Name"].ToString(), "I");
                        }
                        #endregion
