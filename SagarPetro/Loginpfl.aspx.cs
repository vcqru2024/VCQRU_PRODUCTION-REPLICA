...
public partial class Patanjali_Loginpfl : System.Web.UI.Page
{
    ...
    protected void btnlogin_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Email) && !string.IsNullOrEmpty(Password))
        {
           ...
           // DataTable dt = db.LoginVendor("USP_GetPflUserLogin", Email, Otp, LoginPassword); //Patanjali
            DataTable dt = sagar.LoginVendor("USP_GetUserLogin_SP", Email, Otp, LoginPassword); //Sagar
            if (dt.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (Session["UserRole_id"] != null)
                    {
                        ...
                        DataTable dtuser = SQL_DB.ExecuteDataTable("select*from tbl_users_SP where id='" + Session["UserRole_id"].ToString() + "'");
                    ...
                    else
                    {
                        ...
                        dt = SQL_DB.ExecuteDataTable("select * from MenuItem_Sagar_petro m inner join tblMenuAuthendication ma on m.MenuItemID= ma.MenuId where Comp_Id='" + dt.Rows[0]["Comp_ID"].ToString() + "' and RefMenu<>0 order by MenuId asc");
                    ...
                    Response.Redirect("../SagarPetro/dashboard.aspx");
                } 
            }
            else
            {
