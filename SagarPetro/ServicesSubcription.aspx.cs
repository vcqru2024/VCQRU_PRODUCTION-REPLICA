...
public partial class Partner_ServicesSubcription : System.Web.UI.Page
{
    ...
    {
        #region Create Conrol Panel For Services
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                 ...
                 //   pnlTextBox.Controls.Add(lt);
                    ...
                    btnAddTxt.ImageUrl = ProjectSession.absoluteSiteBrowseUrl + "/Data/Data/Service/" + dt.Rows[i]["Image"].ToString(); ;// "../Data/Service/" + dt.Rows[i]["Image"].ToString();
                   ...
                   // pnlTextBox.Controls.Add(btnAddTxt);
                    ...
                    srv.Append("<span style='width:49%; float:left; text-align:left;'>" +
                        "<a href='#'>Know More...</a></span><span style='width:49%; float:right; padding: 1px !important;  text-align: center;'>" +
                        "<p title= '" + dt.Rows[i]["Service_ID"].ToString() + "' onclick='javascript:GetService()' style='border:solid 1px #e6e6e6; " +
                        "background-color:#909090; color:#f0f0f0; cursor:pointer;'>Buy Now</p></span>");
                    srv.Append("</div>");
                    srv.Append("</div>");
                    srv.Append("</div>");
                    srv.Append("</div>");
        ...
        if (Session["CompanyId"] == null)
            Response.Redirect("../patanjali/loginpfl.aspx?Page=ServicesSubcription.aspx");
        else
        {
    ...
    private void FillDropdown()
    {
        ...
        ds = SQL_DB.ExecuteDataSet("SELECT Service_ID, ServiceName, IsShowPrice, EntryDate, Image, IsActive, IsDelete FROM M_Service WHERE Service_ID='SRV1018' and  IsActive = 0 AND IsDelete = 0");
    ...
    protected void Submit_Click(object sender, EventArgs e)
    {
        ...
        {
            Reg.Subscribe_Id = lblproidamc.Value;
            Reg.DML = "U";
        }
    ...
    protected void GrdProductMaster_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "DeleteRow")
        {
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "Updated", "showAlert('Success', 'Product " + Reg.Pro_Name + " has been deleted successfully !', 'success');", true);
        ...
        {
            Response.Redirect("AddServiceSubscription.aspx?id=" + e.CommandArgument.ToString());
    ...
    {
        Response.Redirect("AddServiceSubscription.aspx");
    }

    protected void GrdProductMaster_RowCommand1(object sender, GridViewCommandEventArgs e)
    {
