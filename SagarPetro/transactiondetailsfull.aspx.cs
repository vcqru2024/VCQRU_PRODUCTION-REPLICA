...
public partial class SagarPetro_transactiondetailsfull : System.Web.UI.Page
{
    ...
    protected void btnsearch_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                DataTable dtuser = SQL_DB.ExecuteDataTable("exec USP_GetAssistantMechanicTransactionDetailsForAdmin_SP '" + Session["CompanyId"].ToString() + "','" + dateFrom + "','" + dateTo + "','" + District + "'");
              //  DataTable dtuser = SQL_DB.ExecuteDataTable("exec USP_GetAssistantMechanicTransactionDetailsForAdminnew_SP '" + Session["CompanyId"].ToString() + "','" + dateFrom + "','" + dateTo + "','" + District + "','"+ State + "'");
                if (dtuser.Rows.Count > 0)
                {
