...
public partial class UserControl_BrandLoyaltyAwards : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //lblloginName
            Label lblLogin = (Label)Page.Master.FindControl("lblloginName");
            lblUser = lblLogin.Text;
            FillGrdLoyaltyAwards(lblUser);
    ...
    protected void lnkDownload_Click(object sender, EventArgs e)
    {
        ...
        string filePath = GrdAwards.DataKeys[gvrow.RowIndex].Value.ToString();
        Response.Clear();
    ...
    //    DataSet ds = SQL_DB.ExecuteDataSet("SELECT [RowId],[Comp_ID],[AwardName],[Points],[IsActive],[IsDelete],[Entry_Date],CASE WHEN IsActive = 0 THEN " +
    //        " 'Click for De-Activate' else 'Click for Activate' end as IsActiveMsg , CASE WHEN IsDelete = 1 THEN 'Click for Un-Delete' else 'Click for Delete' end as IsDeleteMsg  FROM [M_LoyaltyAwards] ");
    ...
    //    //    lblcount.Text = "0";
    //}
    private void FillGrdLoyaltyAwards(string lusername)
    {
        //Loyalty_Awards Reg = new Loyalty_Awards();
        //Reg.RowId = 0;
        //Reg.Comp_ID = "";// Session["CompanyId"].ToString();
        //Reg.AwardName = txtsearchlblname.Text.Trim().Replace("'", "''");
        DataSet ds = Warranty.GetWarrantyDetail(lusername);
        if (ds.Tables.Count > 0)
        {
            //if (Convert.ToInt32(ddlRowProductCnt.SelectedValue) > 1000)
            //{
