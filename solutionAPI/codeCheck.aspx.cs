...
public partial class codeCheck : System.Web.UI.Page
{
  ...
  #region Apply New logs for IVR Data status
            string RowData = " | Code1 : " + Code1 + " | Code2 : " + Code2 + " | callercircle : " + callercircle + " | callerno : " + callerno + " | network : " + network + " | date : " + vdate + " | time : " + time;

            string Url = Request.Url.AbsoluteUri;
            IVRDatalogs(callerno, Url, RowData);
            #endregion
 ...
 private static void IVRDatalogs(string Mobile, string Content, string Response)
    {
        try
        {
            string filename = "IVRDatalogsCodeCheck_" + System.DateTime.Today.ToString("yyyy-MM-dd");
        ...
        else if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            if (dsres.Tables.Count > 0)
            {
                if (dsres.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["Use_Count"].ToString() == "0")
                    {
                        ...
                        if (dtServiceAssign.Rows.Count > 0)
                        {
                            ...
                            DataRow[] dr5 = dtServiceAssign.Select("Service_id = 'SRV1024'");
                            if (dr5.Length > 0)
                            {
                                try
                                {
                                    DataTable dtBuild_Loyalty = ServiceLogic.Proc_SaveCodeDtsForBuiltLoyalty(Reg.Received_Code1, Reg.Received_Code2,
                                        Convert.ToInt32(dr5[0]["SST_Id"]), intM_Consumer_MCOde).Tables[0];
                                    if (dtBuild_Loyalty.Rows.Count > 0)
                                    {
                                        string strEarnedPoints = dtBuild_Loyalty.Rows[0]["Points"].ToString();
                                        string IsCashConvert = dtBuild_Loyalty.Rows[0]["IsCashConvert"].ToString();
                                        str_CashTransfer = dtBuild_Loyalty.Rows[0]["IsCash"].ToString();
                                        if (dsres.Tables[0].Rows[0]["Comp_Name"].ToString() == "GRIH PRAWESH MARKETING COMPANY")
                                        {
                                            if (str_CashTransfer == "40")
                                            {
                                                str_Build_Loyalty = "2";
                                            }
                                            else
                        ...
                        else
                        {
                            ...
                            Message Obj1 = new Message();
                            Obj1.Status_Code = "0";
                            //Obj1.Status_Message = "Authenticity of the product with Code1- " + Code1 + " and Code2- " + Code2 + "  can not be guaranteed .";
                            Obj1.Status_Message = "Service of the coupon " + Code1 + Code2 + " has been deactivated.";
                            ans = "{" + pp + ":" + JsonConvert.SerializeObject(Obj1, Formatting.Indented) + "}";
                        ...
                        try
                        {
                            ...
                            Obj.Comment_English_File = DtDetail.Rows[0]["comment_english"].ToString();
                            Obj.Comment_Hindi_File = DtDetail.Rows[0]["comment_hindi"].ToString();
                            if (DtDetail.Rows[0]["Comp_Name"].ToString() == "GRIH PRAWESH MARKETING COMPANY")
                            {
                                int id = 20;
                                DataRow[] dr = dtServiceAssign.Select("Service_id = 'SRV1001'");
                                if (dr.Length > 0)
                                    ...
                                    id = 4;
                                string msg = message(dtServiceAssign.Rows[0]["Service_id"].ToString(), id.ToString(), callerno).ToUpper().Replace("<CODE1>", Reg.Received_Code1).Replace("<CODE2>", Reg.Received_Code2).Replace("<CASH>", str_CashTransfer).Replace("<POINTS>", str_Build_Loyalty);
                                ServiceLogic.SendSmsfromknowlarity(msg, callerno);
                            }
                        }
                        catch (Exception ex)
                        {
                ...
                else
                {
                    ...
                    if (dsres.Tables[0].Rows[0]["Comp_Name"].ToString() == "GRIH PRAWESH MARKETING COMPANY")
                    {
                        ServiceLogic.SendSmsfromknowlarity(Obj.Status_Message, callerno);
                    }
                    return;
    ...
    private string message(string serviceid, string messageid, string mobile)
    {
        return SQL_DB.ExecuteScalar("select MsgBody from M_CodeCheckReturnMessage where Service_ID='" + serviceid + "' and MsgType=" + messageid).ToString();
